Recherchen:
- [ ] Datenbanken (Deutschland):
 * [ ] Gibt es was besseres als Gutenberg Projekt für deutschen Raum?
 * [ ] Bücher?
 * [ ] Zeitungsartikel?
-> wie viele Bücher wollen wir ziehen? Nur die wo Arten vorkommen
-> welches Feld? Lyrik/ Gedichte, (Romane erst 18Jhd)
- [ ] Artenliste als Selektionskriterium
- [ ] Rechtsabteilung im Vorfeld fragen?

- [ ] Variablen in der Datenbank für Tabellen:
 * [ ] AutorInnen
 * [ ] Bücher (als PDFs vorliegend): Genre (was bei Sammlungen?)

- [ ] Gegen Ende -> Validierung: ~ 100 Textteile querlesen, die ein RAG System findet

- [ ] https://digitalhumanities.uni-freiburg.de/dh-lab/ anfragen zwecks Rechner, Ressourcen

- [ ] Juliane fragt Rebecca, ob sie jemand kennt


# Ideen:

## mögliche Paper:
- [ ] Welche zentrale Fragen wollen wir im Artikel diskutieren?

- **Biodiversitäts-Kurven**: Artenliste mit deutschen Namen: Tiere, Pflanzen, Vögel; wie häufig wie viele unterschiedliche Tiere für welche Artengruppen über die Zeit gesehen
 * Trivialnamen für Arten
 * Vögel vs. Pilze, ...
 * Hypothese: Biodiversität ist verloren gegangen, Wissen ist verloren gegangen
- **Wald-Narrative**: Narrative, Gefühle abbilden:
 * Literaturwissenschaftliche untermauern: wide reading ---> close reading
 * Narrativität: wie wird Wald erzählt, welche soziale Bedeutung hat Wald
- Wie wurden Debatten über Konflikte gelöst?
-> Naturbeschreibungen oftmals zu lang, zu unstrukturiert; hier auf ein Thema fokussieren


## Journal:
- wenn Biodiversitäts-Kurven Deutschland mittelhohes Journal
- wenn Biodiversitäts-Kurven mehrere Länder hohes Journal


# Entscheidungen:

- mit einer Sprache starten:
  * für uns deutsche Literatur die wichtigste Literatur
  * schwedische Literaturdatenbank
 = klein starten

- Start mit einer Literaturdatenbank


Publikationsstrategie:
- Future Forest: Wie gestaltet man zukünftiges Waldmanagement unter Unsicherheiten?
   * wichtige Komponente: soziale Seite
   * Auswirkung Ländergrenzen (sozio Faktoren)

 
 weitere Finanzierung: Incubator Fund 2026 Oktober Future Forest


  # Literatur:

 - https://www.nature.com/articles/s41598-024-55742-x
 - https://www.nature.com/articles/s41598-025-28327-5
 - https://hal.science/hal-00302717v1/document
